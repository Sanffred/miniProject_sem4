...
class MainWindowController:
    def __init__(self):
        ...
        self.mainWindow.setWindowTitle("Automated Backup Software")
        
        # Set the window icon
        self.mainWindow.setWindowIcon(QIcon('appLogo2.png'))
    ...
    def startThirdPage(self):
        ...
        # self.thirdPageWindow.ui.localdir_toggle.stateChanged.connect(self.handle_checkbox)
    ...
    ############################ Local Directory backup functions #################################

    # def handle_checkbox(self, states):
    #         if self.states["Local Directory"]:
    #             logging.debug("Checkbox is checked, opening file dialog")
    #             folder = str(QFileDialog.getExistingDirectory(self, "Select Directory"))
    #             if folder:
    #                 # If a directory is selected, log this or perform further actions
    #                 logging.debug(f"Selected directory: {folder}")
    #             else:
    #                 # No directory was selected, uncheck the checkbox
    #                 self.checkbox.setChecked(False)
    #                 logging.debug("No directory selected, checkbox unchecked")
    #         else:
    #             print("Checkbox is unchecked")
    #             logging.debug("Checkbox is unchecked")


            
    ############################# local directory backup functions ends here ########################

    ############################# Dropbox functions start here ##############################
    ...
    def authenticate_dropbox(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            datadir2 = sys._MEIPASS
        else:
            # The application is not frozen
            datadir2 = os.path.dirname(os.path.abspath(__file__))

        token_path = os.path.join(datadir2, 'TOKEN.txt')
        try:
            # Read the access token from TOKEN.txt file
            with open(token_path, 'r') as token_file:
                self.access_token = token_file.read().strip()

            # Use the access token to create a Dropbox object
    ...
    def upload_folder_toDropbox(self, folder_path):
        ...
        self.droppBox_uploaded_folders.append(dropbox_folder_path)  # Store the folder Dropbox path
        ...
        for root, dirs, files in os.walk(folder_path):
            for file in files:
                if file.lower() == 'desktop.ini':
                    ...
                    continue  # Skip the upload of desktop.ini
    ...
    def upload_file_toDropbox(self, file_path, destination_path=None):
        ...
        if destination_path is None:
            ...
            destination_path = f"/{filename}"  # Default to root directory
        ...
        with open(file_path, 'rb') as file:
            try:
                ...
                self.droppBox_uploaded_files.append(destination_path)  # Store the Dropbox path
        ...
        try:
            ...
            if hasattr(self, 'droppBox_uploaded_files'):
                for destination_path in self.droppBox_uploaded_files:
                    if destination_path.startswith("/"):  # Ensure it's a Dropbox path
            ...
            if hasattr(self, 'droppBox_uploaded_folders'):
                for folder_path in self.droppBox_uploaded_folders:
                    if folder_path.startswith("/"):  # Ensure it's a Dropbox path
    ...
    def delete_folder_from_dropbox(self, folder_name):
        ...
        try:
            # Attempt recursive deletion by first deleting all contents
        ...
        try:
            ...
            for item in os.listdir(folder_path):
                ...
                if os.path.isfile(item_path):
                    MainWindowController.upload_file_toGoogle(service, item_path, folder_id)
